...
#############################################################################################################################
sqrt_keyword, sin_keyword, cos_keyword, exp_keyword, log_keyword, rand_keyword, print_keyword = Hulk_G.Terminals('SQRT SIN COS EXP LOG RAND PRINT') 
...
plus, minus, multiply, divide, power,percent = Hulk_G.Terminals('+ - * / ^ %')  
less_than, greater_than, less_equal, greater_equal, equal, not_equal = Hulk_G.Terminals('< > <= >= == !=')
and_op, or_op, not_op = Hulk_G.Terminals('& | !')
...
function_keyword, let_keyword, in_keyword= Hulk_G.Terminals('FUNCTION LET IN')
if_keyword,elif_keyword,else_keyword = Hulk_G.Terminals('IF ELIF ELSE')
true_keyword , false_keyword = Hulk_G.Terminals('TRUE FALSE')
while_keyword, for_keyword = Hulk_G.Terminals('WHILE FOR ')
type_keyword = Hulk_G.Terminal('TYPE')
new_keyword, inherits_keyword = Hulk_G.Terminals('NEW INHERITS')
is_keyword , as_keyword = Hulk_G.Terminals('IS AS')
protocol_keyword,extends_keyword = Hulk_G.Terminals('PROTOCOL EXTENDS')
###############################################################################################################################
line = 'line'
###############################################################################################################################
