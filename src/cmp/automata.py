...
def automata_quantifier(a1, min_val, max_val):
    if not isinstance(min_val, int) or not isinstance(max_val, int):
        raise ValueError(
            f"min_val and max_val must be integers, but got {type(min_val).__name__} and {type(max_val).__name__}"
        )
...
def _get_chars_in_range(start, end):
    ...
    if isinstance(start, str) and isinstance(end, str):
        ...
        raise ValueError(
            "start and end must be both single characters or both integers"
        )

    return [
        chr(symbol) if is_char else str(symbol)
        for symbol in range(start_ord, end_ord + 1)
    ]
