...
expression, expression_block, statements, main_expression, function_decls = Hulk_G.NonTerminals(
    'EXPRESSION EXPRESSION_BLOCK STATEMENTS MAIN_EXPRESSION FUNCTION_DECLS')
...
vector_inst, vector_explicit, vector_implicit, vector_indexing = Hulk_G.NonTerminals(
    'VECTOR_INST VECTOR_EXPLICIT VECTOR_IMPLICIT VECTOR_INDEXING')
...
function_inline %= function_keyword + identifier + parentized_param_list + optional_type_annotation + \
    arrow + expression, lambda h, s: FuncNode(s[2], s[3], s[4], s[6], s[1, line])
...
builtin_function_call %= log_keyword + paren_open + expression + comma + \
...
method_inline %= identifier + parentized_param_list + optional_type_annotation + \
    arrow + expression, lambda h, s: MethodNode(s[1], s[2], s[3], s[5], s[1, line])
...
vector_implicit %= square_bracket_open + expression + double_pipe + identifier + in_keyword + \
    expression + square_bracket_close, lambda h, s: ImplicitVectorNode(s[2], s[4], s[6], s[1, line])
