...
class TypeCollectorVisitor():
    ...
    def visit(self, node: TypeNode):
        try:
            ...
            self.context.types[node.identifier.lex] = ErrorType()
            error_with_line = SemanticError(f"Error at line {node.identifier.line}: {str(e)}")
            self.errors.append(error_with_line)
        
    @visitor.when(FuncNode)
    def visit(self, node: FuncNode):
        try:
            self.context.register_function_name(node.identifier.lex,len(node.params))
        except SemanticError as e:
    ...
    def visit(self, node: ProtocolNode):
        try:
            ...
            self.context.protocols[node.identifier.lex] = ErrorType()
            error_with_line = SemanticError(f"Error at line {node.identifier.line}: {str(e)}")
            self.errors.append(error_with_line)
